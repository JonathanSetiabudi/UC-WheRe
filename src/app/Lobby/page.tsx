...
const Lobby = ({code}) => {
    ...
    // theme handlers
    const handleClickThemeResAndDining = (event: React.MouseEvent<HTMLButtonElement>) => {
        onThemeChange(0);
    }

    const handleClickThemeCampusLandmarks = (event: React.MouseEvent<HTMLButtonElement>) => {
        onThemeChange(1);
    }

    const handleClickThemeStudySpots = (event: React.MouseEvent<HTMLButtonElement>) => {
        onThemeChange(2);
    }

    const handleClickThemeBikeRacks = (event: React.MouseEvent<HTMLButtonElement>) => {
        onThemeChange(3);
    }

    const handleClickThemeStreetsAndParking = (event: React.MouseEvent<HTMLButtonElement>) => {
        onThemeChange(4);
    }
    ...
    const handleClickBoardSmall = (event: React.MouseEvent<HTMLButtonElement>) => {
        onGridChange(16);
    }

    const handleClickBoardLarge = (event: React.MouseEvent<HTMLButtonElement>) => {
        onGridChange(20);
    }
    ...
    return (
        <div>
            ...
            <br></br>
            <p>Select a theme:</p>
            <button onClick={handleClickThemeResAndDining}> Residential and Dining </button>
            <br></br>
            <button onClick={handleClickThemeCampusLandmarks}> Campus Landmarks </button>
            <br></br>
            <button onClick={handleClickThemeStudySpots}> Study Spots </button>
            <br></br>
            <button onClick={handleClickThemeBikeRacks}> Bike Racks </button>
            <br></br>
            <button onClick={handleClickThemeStreetsAndParking}> Streets and Parking Lots </button>
